  ...
  if (contactForm) {
    ...
    function getErrorMessage(field, rule, param) {
      const label = field.closest('.form-group, .field-wrapper, label')
        ? (field.labels && field.labels[0] ? field.labels[0].textContent.trim() : field.name || field.id)
    ...
    function validateField(field) {
      ...
      if (fieldType === 'email' || dataset.validate === 'email') {
      ...
      if (fieldType === 'tel' || dataset.validate === 'phone') {
      ...
      if (dataset.minlength) {
        if (!validators.minLength(value, dataset.minlength)) {
          showError(field, getErrorMessage(field, 'minLength', dataset.minlength));
          return false;
        }
      }
    ...
    "translations": {
      "he": {
        "consentModal": {
          ...
          "description": "ליאל - שיעורים פרטיים במתמטיקה משתמש בעוגיות כדי לשפר את החוויה שלך, לנתח שימוש באתר ולסייע במאמצי השיווק שלנו.",
...
(function() {
    ...
    contactForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            var payload = {
                websiteId: '4ee3ceda-202e-480a-abbb-324455dd74d5',
                name: resolvedName,
                email: resolvedEmail,
            ...
            if (result.success) {
                ...
                if (result.thankYouPagePath && result.ticketNumber) {
                    ...
                    if (isPreview && window.ZAPPY_CONFIG) {
                        var wid = window.ZAPPY_CONFIG.websiteId || '4ee3ceda-202e-480a-abbb-324455dd74d5';
...
(function(){
  try {
    ...
    document.addEventListener('submit', function(e) {
      ...
      var wid = '4ee3ceda-202e-480a-abbb-324455dd74d5';
